...
let make (rsa_pub : Nocrypto.Rsa.pub) : (t, [ `Msg of string ]) result =
  let n =
    CCString.rev (Z.to_bits rsa_pub.n)
    |> trim_leading_null
    |> Base64.encode ~pad:false ~alphabet:Base64.uri_safe_alphabet
  in
  let e =
    CCString.rev (Z.to_bits rsa_pub.e)
    |> trim_leading_null
    |> Base64.encode ~pad:false ~alphabet:Base64.uri_safe_alphabet
  in
  let public_key : X509.public_key = `RSA rsa_pub in
  let kid =
    public_key |> X509.key_id |> Cstruct.to_string
    |> Base64.encode ~pad:false ~alphabet:Base64.uri_safe_alphabet
  in
  let x5t =
    public_key |> X509.key_fingerprint |> Hex.of_cstruct |> Hex.to_bytes
    |> Bytes.to_string
